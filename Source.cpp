...
int create_files_string(int count, int l, string fi)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		//cout << "C:\\Users\\Администратор\\source\\repos\\mthreads\\mthreads\\test\\" + to_string(i) + ".txt" << endl;
		outdata.close();
	}
...
public:
	...
	void someFunctionThatWritesToFile(const string &data) {
		// Do some work...
		_sf->write(data);
	}
private:
...
public:
	...
	void print_to_file()
	{
		...
		*/
		...
		auto synchronizedFile = std::make_shared<SynchronizedFile>(out_dir);
		...
		while ((done < (_files.size() - 1)) || (temp_size > 0))
		{
			while (!_out.empty())
			{
				...
				for (string ss : _out.front())
				{
					writer1.someFunctionThatWritesToFile(ss);
				}
				cout << "id = " << get_id() << "    all=" << _files.size() << "    :   done= " << done << "   :    read= " << temp_size << endl;
				std::thread::id xqz = get_id();
				done++;
				_out.pop();
			}
		}
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		if ((argc == 2) || ((string)argv[1] == "-h") || ((string)argv[1] == "-help") || ((string)argv[1] == "-?"))
		{
			...
			cout << " ./mthreads -thread 1 " << setw(20) << " Parsing strings in files from source_folder and writing data to destionation_file using 1 thread!" << endl;
			cout << " ./mthreads -thread 3 " << setw(20) << " Parsing strings in files from source_folder and writing data to destionation_file using 3 threads (reading + parsing + writing)!" << endl;
		...
		else
		{
			if ((string)argv[1] == "-create_files")
			{
				...
				else
				{
					cout << "Hello! It's help menu" << endl;
					cout << "Using: \n ./mthreads -h " << setw(20) << "Help menu" << endl;
					cout << " ./mthreads -create_files <count> <length of string> <count of delimetres> -folder <folder_name> " << setw(20) << " Creating files in destination folder with set params" << endl;
					cout << " ./mthreads -transfer <source_folder> <destination_file> " << setw(20) << " Parsing strings in files from source_folder and writing data to destionation_file" << endl;
					cout << " ./mthreads -thread 1 " << setw(20) << " Parsing strings in files from source_folder and writing data to destionation_file using 1 thread!" << endl;
					cout << " ./mthreads -thread 3 " << setw(20) << " Parsing strings in files from source_folder and writing data to destionation_file using 3 threads (reading + parsing + writing)!" << endl;
			...
			if ((string)argv[1] == "-thread")
			{
				...
				else if ((string)argv[1] == "3")
				{
					...
					std::thread t(&Bar::r, &test);
					t.detach();
					std::thread t1(&Bar::wr, &test);
					t1.detach();
					test.print_to_file();
	...
	for (int i = 0; i < std::thread::hardware_concurrency() * 2; i++)
	{
		ThreadVector_write_to_file.push_back(async(&Bar::print_to_file, &test));
	}
	...
	for (auto &t2 : ThreadVector_write_to_file)
	{
		t2.get();
	}
